...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
rootInputFolder='C:\Users\OCT\OneDrive - Queen Mary, University of London\Shared OCT Data\BSU - November 2015\BSU2015 -  Day 1\Time-lapse 4';
...
for r=startRepeat:skipRepeat:endRepeat
    ...
    for p=startPosition:skipPosition:endPosition
       ...
       % outMeanAScanFolder = [outPositionFolder '/' outMeanAScanFolderTitle];
        ...
        disp('Done.');
        %
        % Now create the OCT image volume A-Scan by A-Scan
        %
        linearVolume=zeros(ascanLength,numAScansPerBScan,numBScans);
        smoothedLinearVolume=zeros(ascanLength,numAScansPerBScan,numBScans);        logVolume=zeros(ascanLength,numAScansPerBScan,numBScans);
        correlationVolume=zeros(ascanLength,numAScansPerBScan,numBScans);        logVolume=zeros(ascanLength,numAScansPerBScan,numBScans);
        for b=1:numBScans
           ...
           for a=1:numAScansPerBScan
               ...
               if ((a>kernelX) && (b>kernelY))
                   
                   for z=(kernelZ+1):ascanLength
                       
                       aa=a-kernelX;
                       bb=b-kernelY;
                       zz=z-kernelZ;
                       kernel=linearVolume(zz:z,aa:a,bb:b);
                       smoothedLinearVolume(zz,aa,bb)=mean(mean(mean(kernel)));
                       %
                       % Correlation mapping on original data (not the
                       % mean)
                       %
                        kernel1=squeeze(linearVolume(zz:z,aa,bb));
                        kernel2=squeeze(linearVolume(zz:z,aa,bb+1));
                        correlationVolume(zz,aa,bb)=corr2(kernel1,kernel2);
                   end
               end
           ...
           if (b>kernelY)
               bb=b-kernelY;
                dlmwrite([outMeanBScanFolder '/' 'meanBScan_t=' repeatString '_b=' num2str(bb) '.csv'],smoothedLinearVolume(:,:,bb));
                dlmwrite([outMeanBScanFolder '/' 'correlationBScan_t=' repeatString '_b=' num2str(bb) '.csv'],correlationVolume(:,:,bb));
           end
        ...
        % Done processing
        %
        disp('Processing complete, saving image files...');
        
        logVol=20*log10(smoothedLinearVolume);

       minVal=min(min(min(smoothedLinearVolume)));
       maxVal=max(max(max(smoothedLinearVolume)));

       minLogVal=-50; %min(min(min(logVol)));
       maxLogVal=25; %max(max(max(logVol)));

       maxInt=65535;
       scaledVol=(smoothedLinearVolume-minVal)/(maxVal-minVal)*maxInt;

       scaledVol(scaledVol>maxInt)=maxInt;
       scaledVol(scaledVol<0)=0;

       imgVol=uint16(scaledVol);

       
       scaledLogVol=(logVol-minLogVal)/(maxLogVal-minLogVal) * maxInt;
       scaledLogVol(scaledLogVol>maxInt)=maxInt;
       scaledLogVol(scaledLogVol<0)=0;
       imgLogVol=uint16(scaledLogVol);
       
       scaledCorrVol=(correlationVolume+1)/2 * maxInt;
       scaledCorrVol(scaledCorrVol>maxInt)=maxInt;
       scaledCorrVol(scaledCorrVol<0)=0;
       imgCorrVol=uint16(scaledCorrVol);
       
       for b=(kernelY+1):numBScans
           

               bb=b-kernelY;
             disp('Writing mean b-scans to file...');
             
             imwrite(imgVol(:,:,bb),[outMeanBScanFolder '/' 'linMeanBScan_t=' repeatString '_b=' num2str(bb) '.png']);
             imwrite(imgLogVol(:,:,bb),[outMeanBScanFolder '/' 'logMeanBScan_t=' repeatString '_b=' num2str(bb) '.png']);
             imwrite(imgCorrVol(:,:,bb),[outMeanBScanFolder '/' 'correlationBScan_t=' repeatString '_b=' num2str(bb) '.png']);
             
        end
        

        %
        % Create an image stack using the volumetric rolling mean
        %
